    ...
    return (
        <div className="fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center">
            ...
            className="relative bg-white p-10 rounded-xl text-slate-500">
                ...
                <p className="text-sm">Welcome to Imagify! Please Login to continue.</p>

                {state !== 'Login' && <div className="border px-6 py-2 flex items-center gap-2 rounded-full mt-5">
                    <img src={assets.profile_icon} alt="" className="w-5 h-5 object-contain"/> 
                    <input onChange={e => setName(e.target.value)} value={name} type='text' placeholder="Username" required className="outline-none text-sm"/>
                </div> }
                ...
                <button className="bg-blue-600 w-full text-white py-2 rounded-full cursor-pointer mt-4">{state === 'Login' ? 'Login' : 'Create account'}</button>
               ...
               { state === 'Login' ? <p className="mt-5 text-center">Don't have an account?  <span className="text-blue-600 cursor-pointer ml-1" onClick={()=> setState('Sign Up')}>Sign up</span></p>
                 ...
                 : <p className="mt-5 text-center">Already have an account?  <span className="text-blue-600 cursor-pointer ml-1" onClick={()=> setState('Login')}>Login</span></p>
               }
